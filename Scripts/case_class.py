	...
	def __init__(self, enc_file_name, dec_file_name):
		...
		while line != '':
			if re.search('Net', line):
				if len(self.case_set_) > 0:
					'''
					#A new client emerges, summarize the condition of last client.
					#target_bitrate is replaced by is_rate_reasonable, real_bitrate is replaced by bitrate_dif(new criterion)
					case.client_[-1].data_['target_purposed_ratio'] = case.client_[-1].find_rate_ratio()
					case.client_[-1].data_['unqualified_ratio'] = case.client_[-1].compare_target_and_real()
					'''
